...
use crate::site::{PageKind, Site, route_from_urlish};
...
fn iter_schema_objects(payload: &Value, depth: usize) -> Vec<(Value, usize)> {
    let mut found = Vec::new();
    match payload {
        Value::Object(map) => {
...
fn is_docs_like_route(route: &str) -> bool {
...
fn profile_prefers_app(profile: &str) -> bool {
    let profile = normalized_profile(profile);
    profile.contains("app") || profile.contains("software") || profile.contains("product")
}

fn infer_schema_page_intent(page: &crate::site::Page) -> SchemaPageIntent {
...
fn collect_schema_page_policy_findings(
    ...
    if config.require_schema_title_alignment && !schema_types.is_empty() {
        ...
        if !visible_titles.is_empty() && !any_visible_title_matches(schema_titles, &visible_titles)
        {
            findings.push(finding(
                "SCH005",
...
fn page_supports_software_application(page: &crate::site::Page) -> bool {
...
fn detail_page_looks_docs_shaped(page: &crate::site::Page) -> bool {
    if is_docs_like_route(&page.route) {
        return true;
    }
    let mut text = String::new();
...
fn software_application_schema_looks_app_shaped(schema_objects: &[SchemaObject]) -> bool {
...
fn should_enforce_required_fields(
    page: &crate::site::Page,
    profile: &str,
    object_type: &str,
    schema_object: &serde_json::Map<String, Value>,
