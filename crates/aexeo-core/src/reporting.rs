...
pub fn write_audit_artifact(
    ...
    let artifact_dir = base_dir.join(".aexeo-reports");
...
mod tests {
    ...
    #[test]
    fn renders_sarif() {
        let payload = render_sarif(&[sample_finding()], "aexeo").unwrap();
    ...
    fn writes_partial_audit_artifact_without_history() {
        ...
        assert!(
            !temp_dir
                .path()
                .join(".aexeo-reports/crawl-trends.json")
                .exists()
        );
    }
    ...
    fn writes_progress_audit_artifact_to_progress_path() {
        ...
        assert!(latest.exists());
        assert!(latest.ends_with(".aexeo-reports/crawl-progress-latest.json"));
    }
