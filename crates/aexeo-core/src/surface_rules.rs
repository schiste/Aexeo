...
pub fn run_surface_rules(site: &Site, config: &Config) -> Vec<Finding> {
    ...
    if !graph.coverage.facts_present {
        findings.push(sitewide_finding(
            ...
            Some(
                "generate a first draft with `aexeo-cli intelligence facts generate --deploy-location root` or `aexeo-cli generate machine-bundle`"
                    .to_string(),
            ),
        ));
    }
    ...
    if graph.coverage.total_routes > 0 && graph.coverage.routes_with_markdown_mirror == 0 {
        findings.push(sitewide_finding(
            ...
            Some(
                "generate deployable mirrors with `aexeo-cli generate markdown-pages --write-dir <public-root>`"
                    .to_string(),
            ),
        ));
    }
