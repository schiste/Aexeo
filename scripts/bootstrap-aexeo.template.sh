#!/bin/sh
# bootstrap-aexeo.sh — vendored bootstrap for the Aexeo CLI.
#
# WHAT THIS DOES:
#   1. Reads .aexeo-version (caret or exact pin, e.g. "^0.7" or "v0.7.0").
#   2. Reads .aexeo-version.lock if it exists (resolved tag, e.g. "v0.7.4").
...
#   4. Downloads the binary for $(uname) into $HOME/.cache/aexeo/<tag>/.
...
#   $AEXEO_CACHE_DIR    — defaults to $HOME/.cache/aexeo.
#   $CI                  — when "true", refuses to mutate .aexeo-version.lock.
#
# BUMPING THE PIN:
#   1. Edit .aexeo-version (e.g. ^0.7 → ^0.8).
#   2. rm .aexeo-version.lock
#   3. ./scripts/bootstrap-aexeo.sh
#   4. git add .aexeo-version .aexeo-version.lock && commit.
set -eu

REPO="${AEXEO_RELEASE_REPO:-schiste/Aexeo}"
CACHE_DIR="${AEXEO_CACHE_DIR:-$HOME/.cache/aexeo}"
HERE=$(CDPATH= cd -- "$(dirname "$0")/.." && pwd)
VERSION_FILE="$HERE/.aexeo-version"
LOCK_FILE="$HERE/.aexeo-version.lock"

err() { printf '%s\n' "$*" >&2; }
die() { err "bootstrap-aexeo: $*"; exit 1; }
...
[ -n "$CONSTRAINT" ] || die ".aexeo-version is empty"
...
if [ -z "$TAG" ]; then
    if [ "${CI:-}" = "true" ]; then
        die "lockfile $LOCK_FILE missing or stale for constraint '$CONSTRAINT'; run bootstrap-aexeo.sh locally and commit the updated lock"
    fi
...
'
    for c in $CANDIDATES; do
        # Skip tags that are not aexeo-cli releases. The repo also
...
INSTALL_BIN="$INSTALL_DIR/aexeo-cli"
